...
---@private
---@param currentPoses any
---@param keyframes table
---@param time any
function Animator:_getNextPose(currentPoses, keyframes, time)
    local nextPoses, newEntry = {}, false
...
function Animator:_initTrack(track)
    ...
    track._currentPoses = track.Keyframes[1].Poses

    local nextPoses = self:_getNextPose(track._currentPoses, track.Keyframes)
    track._targetPoses = nextPoses
    track._reachedKeyframe = {}
end
...
function Animator:_step(dt)
    for track, _ in pairs(self._playingTracks) do
        for motor6d, pose in pairs(track._targetPoses) do
            ...
            math.min(
                1,
                easingStyle(track.TimePosition - track._timeMap[track._currentPoses[motor6d]]) /
                track._timeMap[pose]
            )

            -- interpolate currentpose to targetpose
            motor6d.Transform = track._currentPoses[motor6d].CFrame:lerp(pose.CFrame, intermediateTime)


        end
        track._targetPoses = self:_getNextPose(track._currentPoses, track.Keyframes, track.TimePosition)
...
function Animator:addPlayingTrack(track)
    ...
    for motor6d, pose in pairs(track._currentPoses) do
